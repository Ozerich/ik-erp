            ...
            foreach ($_POST['printArr'] as $id) {
                ...
                <table border="1" style="margin-bottom 20px; border-color: black;border-collapse: collapse;">
                    <tr>
                        ...
                        <td style="background: #bfbfbf; font-size: 18px; text-align: center">Заказ № <?=$id?>
                            от <?=$dateStart[2]?>.<?=$dateStart[1]?></td>
                        <td style="background: #bfbfbf; font-size: 18px; text-align: center"
                            colspan="2"><?=$this->getDivision($order->division)?>,<?=$order->customer?></td>
                        <td style="background: #bfbfbf; font-size: 18px;  text-align: center"
                            colspan="4"><?=$this->needInstall($order)?></td>
                    </tr>
                    <tr>
...
<div>
    <form method="POST" id="print">
        ...
        <input type="submit" class="btn btn-primary" style="float: right" onclick="window.print(); return false;"
               value="Печать документа">
    </form>
</div>
