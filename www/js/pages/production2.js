...
function Order() {
    ...
    this.shipping_date_day_str = ko.computed(function () {
        if (!this.fact_shipping_date()) {
            return ''
        }
        return App.Helper.getWeekDayName(this.fact_shipping_date());
    }, this);
...
function PageViewModel() {
    ...
    this.statuses = [
        ...
        {id: (this.active_page_tab()!=1)?1:0, name: (this.active_page_tab()!=1)?'В работе':'Отменить заказ'},
    ...
    this.filtered_date_orders = ko.computed(function () {
        ...
        return ko.utils.arrayFilter(this.orders(),function (order) {
            return order.date() <= date_end && order.status() == 1;
...
$(document.body).ready(function(e)
{
    ...
    /*$(document).on('click','.date',function(e){
        $(this).find('span:contains("'+month+'")').parent().css('color','white');
        return false;
    });
    $('.date').click();*/
    $(document).delegate(".date", "ready", function(){
        $(this).find('span:contains("'+month+'")').parent().css('color','white');
    });

    /*$(document).delegate("li:contains('Отменить заказ')", "click", function(){
        alert(1);
    });*/
});